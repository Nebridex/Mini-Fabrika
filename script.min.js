(function(){
  ...
  var PAYLOAD_TTL_MS=60*60*1000;
  var MAX_INLINE_STL_BYTES=10*1024*1024;
  var MAX_STORED_PAYLOAD_BYTES=350000;
  var NOTES_AUTO_PREFIX='[MF_AUTO]';
  var MIN_FORM_FILL_MS=3000;
  document.addEventListener('DOMContentLoaded',function(){
    var cfg=window.__MF_CONFIG||{};
    var transferLog=((cfg.transferLogEndpoint||window.__MF_TRANSFER_LOG_ENDPOINT||'')).trim();
    var minFormFillMs=typeof cfg.minFormFillMs==='number'?cfg.minFormFillMs:MIN_FORM_FILL_MS;
    var requireUserInteraction=cfg.requireUserInteraction!==false;
    ...
    qsa('form[data-formsubmit], form[action*="formsubmit.co"]').forEach(function(form){
      var startTs=Date.now();
      var startInput=qs('#formOpenMs',form);
      if(startInput) startInput.value=String(startTs);
      var interacted=!requireUserInteraction;
      form.addEventListener('focusin',function(){interacted=true;});
      form.addEventListener('input',function(){interacted=true;});
      form.addEventListener('change',function(){interacted=true;});
      form.addEventListener('submit',function(e){
        if(requireUserInteraction&&!interacted){
          e.preventDefault();
          alert('Lütfen form alanlarını doldurup tekrar gönderin.');
          return;
        }
        if(Date.now()-startTs<minFormFillMs){
          e.preventDefault();
          alert('Lütfen formu kontrol edip birkaç saniye içinde tekrar gönderin.');
          return;
        }
        var honey=qs('input[name="_honey"]',form);
        if(honey&&honey.value.trim()!==''){e.preventDefault();return;}
        var required=[
          {id:'fullName',msg:'Lütfen ad soyad girin.'},
          {id:'email',msg:'Lütfen e-posta adresi girin.'},
          {id:'modelLink',msg:'Lütfen model linkini girin.'},
          {id:'quantity',msg:'Lütfen adet girin.'}
        ];
    ...
    function showTransferMeta(meta){
      var box=qs('[data-transfer-meta]');
      if(!box) return;
      var params=new URLSearchParams(window.location.search);
      if(params.get('debug_transfer')!=='1') return;
      if(!meta||!meta.source) return;
      box.textContent='Aktarım kaynağı: '+meta.source+' '+(meta.version||'');
      box.hidden=false;
      logTransfer('meta-shown',meta);
    }
    function readLocalPayload(){
      try{
        ...
        if(raw.length>MAX_STORED_PAYLOAD_BYTES){
          ...
          logTransfer('payload-too-large-local',{});
          return null;
        }
        var tsRaw=localStorage.getItem(PAYLOAD_TS_KEY);
        if(tsRaw){
          var age=Date.now()-parseInt(tsRaw,10);
          if(!isNaN(age)&&age>PAYLOAD_TTL_MS){
            logTransfer('payload-expired',{ageMs:age});
            localStorage.removeItem(PAYLOAD_KEY);
            localStorage.removeItem(PAYLOAD_TS_KEY);
            return null;
          }
    ...
    function clearTransferFields(){
      ...
      var modelUrlLink=qs('[data-model-url]');
      if(modelUrlLink){
        modelUrlLink.hidden=true;
        modelUrlLink.removeAttribute('href');
      }
      var metaBox=qs('[data-transfer-meta]');
      if(metaBox){metaBox.hidden=true;metaBox.textContent='';}
    }
    function resolveModelUrl(payload){
      var p=payload||{};
      return getPath(p,['model_url','modelUrl','stl.model_url','stl.modelUrl','stl.uploadUrl','upload.url']);
    }
    function prepareStlDownload(payload){
      ...
      var stlBtn=qs('[data-stl-download]');
      var modelUrlLink=qs('[data-model-url]');
      if(!stlBtn&&!stlBanner&&!modelUrlLink) return;
      ...
      var modelUrl=resolveModelUrl(p);
      var safeModelUrl=safeHttpUrl(modelUrl);
      if(stl.tooLarge===true){
        logTransfer('stl-too-large',{sizeBytes:stl.sizeBytes||0,hasModelUrl:!!safeModelUrl});
        if(stlBanner){
          stlBanner.textContent='Model dosyası büyük. Operasyon panelinden backend upload ile alın.';
          stlBanner.hidden=false;
        }
        if(safeModelUrl&&modelUrlLink){
          modelUrlLink.href=safeModelUrl;
          modelUrlLink.hidden=false;
        }
        return;
      }
      if(stl.sizeBytes&&stl.sizeBytes>MAX_INLINE_STL_BYTES){
        logTransfer('stl-inline-limit',{sizeBytes:stl.sizeBytes,limit:MAX_INLINE_STL_BYTES,hasModelUrl:!!safeModelUrl});
        if(stlBanner){
          stlBanner.textContent='Model dosyası büyük. Operasyon panelinden backend upload ile alın.';
          stlBanner.hidden=false;
        }
      ...
      try{
        var stlText=decodeBase64Utf8(stl.base64);
        ...
        logTransfer('stl-ready',{sizeBytes:stl.sizeBytes||0});
      }catch(_e){
        logTransfer('stl-error',{});
        if(stlBanner){
    ...
    function fillTeklifFromPayload(payload){
      ...
      if(noteField&&notes.length){
        ...
        var noteBlock=NOTES_AUTO_PREFIX+'\n'+notes.join('\n');
        var cleanExisting=existing.replace(/\n?\[MF_AUTO\][\s\S]*$/,'').trim();
        noteField.value=(cleanExisting?cleanExisting+'\n\n':'')+noteBlock;
      }
      var banner=qs('[data-design-prefill-banner]');
    ...
    if(window.location.pathname.indexOf('teklif')!==-1){
      ...
      if(!payload){payload=readQueryPayload();if(payload)logTransfer('query-fallback-used',{});}
      fillTeklifFromPayload(payload);

      var clearBtn=qs('[data-clear-teklif]');
      if(clearBtn){
        clearBtn.addEventListener('click',function(){
          try{localStorage.removeItem(PAYLOAD_KEY);localStorage.removeItem(PAYLOAD_TS_KEY);}catch(_e){}
          clearTransferFields();
          logTransfer('transfer-cleared',{});
        });
      }
    }
