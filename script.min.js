(function(){
  function qs(s,c){return(c||document).querySelector(s)}
  function qsa(s,c){return Array.from((c||document).querySelectorAll(s))}

  function setupMenu(){
    var toggle=qs('.menu-toggle');var nav=qs('nav');
    if(!toggle||!nav)return;
    toggle.addEventListener('click',function(){
      var open=nav.classList.toggle('open');
      toggle.setAttribute('aria-expanded',open?'true':'false');
    });
  }

  function setupFilters(){
    var params=new URLSearchParams(location.search);
    var defaultCategory=params.get('category');
    var buttons=qsa('.filter-btn');
    if(!buttons.length)return;
    function apply(key){
      buttons.forEach(function(b){b.classList.toggle('active',b.dataset.filter===key)});
      qsa('.product-card,.blog-card').forEach(function(card){
        card.style.display=(key==='all'||card.dataset.category===key)?'flex':'none';
      });
      if(key!=='all'){params.set('category',key)}else{params.delete('category')}
      history.replaceState(null,'',location.pathname+(params.toString()?('?'+params.toString()):''));
    }
    buttons.forEach(function(btn){btn.addEventListener('click',function(){apply(btn.dataset.filter)})});
    if(defaultCategory&&buttons.some(function(b){return b.dataset.filter===defaultCategory;})){apply(defaultCategory)}
  }

  function setupQuoteForm(){
    var form=qs('#quoteForm'); if(!form)return;
    var params=new URLSearchParams(location.search);
    var modelName=params.get('modelName')||params.get('model');
    var modelUrl=params.get('modelUrl')||params.get('link');
    var category=params.get('category');
    var modelInput=qs('#modelLink'); var notesInput=qs('#notes');
    var prefill=qs('[data-prefill-banner]');
    if(modelUrl&&modelInput)modelInput.value=modelUrl;
    if((modelName||category)&&notesInput){
      var note='';
      if(modelName)note+='Model: '+modelName+'\n';
      if(category)note+='Kategori: '+category+'\n';
      notesInput.value=(note+notesInput.value).trim();
    }
    if(prefill&&(modelName||modelUrl||category))prefill.hidden=false;

    form.addEventListener('submit',function(e){
      e.preventDefault();
      var email=qs('#email').value.trim();
      var phone=qs('#phone').value.replace(/\D/g,'');
      var quantity=qs('#quantity').value;
      var model=qs('#modelLink').value.trim();
      var warn=qs('#urlWarning'); warn.hidden=true;
      if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){alert('Geçerli bir e-posta girin.');return;}
      if(phone && phone.length<10){alert('Telefon en az 10 haneli olmalı.');return;}
      if(!/^https?:\/\//i.test(model)){alert('Model linki geçerli bir URL olmalı.');return;}
      if(!/^\d+$/.test(quantity)||parseInt(quantity,10)<1){alert('Adet en az 1 olmalı.');return;}
      if(!/(makerworld|printables|drive\.google)/i.test(model)){warn.hidden=false;}

      var fullName=qs('#fullName').value.trim();
      var service=qs('#serviceType').value;
      var material=qs('#material').value;
      var notes=qs('#notes').value.trim();
      var body='Merhaba MiniFabrika,%0A%0ATeklif detaylarım:%0A'+
        'Ad Soyad: '+encodeURIComponent(fullName)+'%0A'+
        'E-posta: '+encodeURIComponent(email)+'%0A'+
        'Telefon: '+encodeURIComponent(qs('#phone').value)+'%0A'+
        'Hizmet: '+encodeURIComponent(service)+'%0A'+
        'Malzeme: '+encodeURIComponent(material)+'%0A'+
        'Adet: '+encodeURIComponent(quantity)+'%0A'+
        'Model Linki: '+encodeURIComponent(model)+'%0A'+
        'Notlar: '+encodeURIComponent(notes);
      qs('#mailtoLink').href='mailto:info@minifabrika.com?subject=MiniFabrika%20Teklif%20Talebi&body='+body;
      qs('#waLink').href='https://wa.me/905456900094?text='+decodeURIComponent(body);
      qs('#submitOptions').hidden=false;
    });
  }

  document.addEventListener('DOMContentLoaded',function(){
    setupMenu();
    setupFilters();
    setupQuoteForm();
  });
})();
